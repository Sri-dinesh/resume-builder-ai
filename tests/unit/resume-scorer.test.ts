...
describe("ResumeScorer", () => {
  it("analyze_with_job_description_returns_keyword_metrics_and_competitive_score", () => {
    ...
    expect(analysis.verdict).toMatch(
      /Strong Match|Competitive|Needs Work|High Risk/,
    );
  ...
  it("analyze_across_50_plus_resumes_produces_stable_non_throwing_results", () => {
    const results = sampleResumeTexts.map((text) =>
      new ResumeScorer(text).analyze(),
    );

    expect(results).toHaveLength(60);
    expect(
      results.every((result) => result.score >= 0 && result.score <= 100),
    ).toBe(true);
  });
});
